    ...
    int N, int K, int D)
{
    ...
    if (i < N) {
        /* FIX 1 — cache point coordinates in registers.
         *
         * BEFORE (buggy): data_t[d*N+i] was read K times in the distance
         * loop (once per centroid, for each dimension d) and then 1 more
         * time in the accumulation loop below — D*(K+1) = 40*9 = 360 L2
         * reads per point instead of the minimum 40.
         *
         * AFTER (fixed): load all D dimensions once into the local array
         * pt[], which the compiler will keep in registers (D=40 fits in the
         * 255 scalar registers available per thread on sm_89).
         * Total L2 reads per point: D = 40, a 9x reduction in memory traffic.
         *
         * The reads data_t[d*N+i] for d=0..D-1 are coalesced as before:
         * adjacent threads i and i+1 read adjacent addresses d*N+i and
         * d*N+(i+1) for the same d — one cache line per warp per dimension. */
        float pt[D]; /* register-resident copy of point i's D coordinates */
        for (int d = 0; d < D; d++)
            pt[d] = data_t[d * N + i]; /* single coalesced read pass over D */
        ...
        for (int k = 0; k < K; k++) {
            float dist = 0.0f;
            for (int d = 0; d < D; d++) {
                float diff = pt[d] - s_centroids[k * D + d]; /* register read, free */
                dist += diff * diff;
            }
            if (dist < min_dist) {
                min_dist = dist;
        ...
        for (int d = 0; d < D; d++)
            atomicAdd(&s_new_centroids[nearest * D + d], pt[d]); /* register read, free */
    }
    __syncthreads();

    /* FIX 2 — remove the incorrect != 0.0f guard before the global atomicAdd.
     *
     * BEFORE (buggy): the guard was meant to skip empty shared buckets and
     * save a global atomicAdd. But it silently drops any block whose points
     * sum to exactly 0.0 in some dimension — a valid floating-point value.
     * Example: a block with 128 points at cluster j, all with coordinate 0.0
     * in dimension d. s_new_centroids[j*D+d] = 0.0 → guard fires → the
     * global accumulator never receives this block's contribution → the final
     * centroid coordinate is computed over fewer points than it should be.
     *
     * AFTER (fixed): always call atomicAdd unconditionally.
     * Cost: at most K*D = 320 extra global atomicAdds per block for the cold
     * path (all-zero buckets). With N/BLKDIM = ~2000 blocks for N=500K, that
     * is 2000*320 = 640K atomicAdds — all adding 0.0, which the hardware
     * handles in a single L2 transaction per address. Negligible vs the
     * N*D = 20M shared-memory atomicAdds already done in the hot path.
     *
     * The guard on s_counts is kept because counts[j]==0 is a genuinely
     * unambiguous signal: if no thread incremented s_counts[j], no point
     * was assigned to cluster j in this block, so there is truly nothing
     * to commit. Unlike floating-point sums, an integer count of 0 cannot
     * arise from cancellation. */
    for (int idx = tid; idx < K * D; idx += blockDim.x)
        atomicAdd(&new_centroids[idx], s_new_centroids[idx]); /* always commit */
    for (int idx = tid; idx < K; idx += blockDim.x) {
        if (s_counts[idx] > 0)
            atomicAdd(&counts[idx], s_counts[idx]); /* guard is safe for integer counts */
    }
}
...
int main( int argc, char *argv[] )
{
    ...
    float h_shifts[32]; // CPU array to fetch the 8 shifts
    int iter = 0;
