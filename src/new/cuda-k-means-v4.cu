...
// --------------------------------------------------------------------------
// KERNEL 1: Classification & Reduction (NOT Transposed / Row-Major)
// --------------------------------------------------------------------------
__global__ void kmeans_classify_and_reduce_kernel(
    const float* __restrict__ data_row_major, // Uncoalesced source
    ...
    int N, int K, int D)
{
    ...
    if (i < N) {
        ...
        for (int k = 0; k < K; k++) {
            float dist = 0.0f;
            for (int d = 0; d < D; d++) {
                // WARNING: Uncoalesced access. Memory jumps by D floats between threads!
                float point_val = data_row_major[i * D + d]; 
        ...
        for (int d = 0; d < D; d++) {
            atomicAdd(&s_new_centroids[nearest * D + d], data_row_major[i * D + d]);
        }
    }
    __syncthreads();

    for (int idx = tid; idx < K * D; idx += blockDim.x) {
        if (s_new_centroids[idx] != 0.0f) atomicAdd(&new_centroids[idx], s_new_centroids[idx]);
    }
    for (int idx = tid; idx < K; idx += blockDim.x) {
...
// --------------------------------------------------------------------------
// KERNEL 2: Update Phase (Same as Version 1)
// --------------------------------------------------------------------------
    ...
    int K, int D)
{
    ...
    for (int d = tid; d < D; d += blockDim.x) {
        ...
        if (count > 0) new_val = new_val / (float)count;
    ...
    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
        if (tid < stride) s_diff[tid] += s_diff[tid + stride];
        __syncthreads();
    }
    
    if (tid == 0) shifts[j] = s_diff[0];
}
...
int main( int argc, char *argv[] )
{
    ...
    // NO TRANSPOSE: Using standard row-major structure
    centroids = (float*)safe_malloc(n_clusters * n_dims * sizeof(*centroids));
    cluster_of = (int*)safe_malloc(n_points * sizeof(*cluster_of));
    init_centroids();

    float *d_data, *d_centroids, *d_new_centroids, *d_shifts;
    int *d_counts, *d_cluster_of;

    // Allocate on device
    CHECK_CUDA(cudaMalloc((void**)&d_data, n_points * n_dims * sizeof(float)));
    ...
    CHECK_CUDA(cudaMalloc((void**)&d_shifts, n_clusters * sizeof(float)));

    // Copy original uncoalesced data
    CHECK_CUDA(cudaMemcpy(d_data, data, n_points * n_dims * sizeof(float), cudaMemcpyHostToDevice));
    ...
    do {
        ...
        kmeans_classify_and_reduce_kernel<<<gridClassify, blockClassify, shared_mem_classify>>>(
            d_data, d_centroids, d_cluster_of, d_new_centroids, d_counts, n_points, n_clusters, n_dims
        );
    ...
    free(data); free(centroids); free(cluster_of);
